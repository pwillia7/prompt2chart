// Ported verbatim from supabase/functions/_shared/llm-adapter.ts (Phase 3).
// These system prompts drive every generated chart — keep concise; do NOT use
// markdown backticks inside the template literals (they close the string).
import type { DatasetSchema, ChartLibrary } from '@/types'
import type { AllSchemaEntry } from './types'

export function formatOtherSchemas(currentSchema: DatasetSchema, allSchemas?: AllSchemaEntry[]): string {
...
export const SYSTEM_PROMPT_VEGA = `You are an expert data visualization assistant. Your task is to generate Vega-Lite specifications based on user prompts and dataset schemas.
...
export const SYSTEM_PROMPT_D3 = `You are an expert D3.js visualization developer. Generate D3.js v7 code for beautiful, interactive SVG visualizations.
...
export const SYSTEM_PROMPT_INSIGHTS = `You are a senior data analyst preparing a dashboard for a stakeholder.
...
export function buildSchemaDescription(schema: DatasetSchema): string {
